...
testing <- (1:168)[-training]
training_full <- c()
for (i in training) {
  print(i)
  print(seq(50 * i, 50*(i+1), by=1))
  training_full[(50*i-1):i*50] <- seq(50 * (i - 1), 50*i, by=1) 
}
 
testing_full <- sapply(testing, function(x) rep(x, 50))
# 
features <- 61 * 61
dataset.size <- 168 * 50
# 
...
train.x <- nn.data.x[training, ]
train.y <- nn.data.y[training] 

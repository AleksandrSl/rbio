...
dds2 <- makeExampleDESeqDataSet(m=4)  # Почему блин в примере все работает, а на практике нет
dds2 <- DESeq(dds2)
resultsNames(dds2)
res <- results(dds2, contrast=c("condition","B","A"))

tissues <- results(dds, contrast = c("tissue", "tissue..NBP", "tissue..breast.tumor"))
cells <- results(dds, contrast = c("cells", "cell.type..Regulatory.T.cells", "cell.type..Conventional.CD4.T.cells"))
cell <- results(dds, name = c("cellscell.type..Regulatory.T.cells"))  # Зато такой вариант работает, но непонятно что он сравнивает
...
cell_types_data <- data.frame(
  ...
  y = -log10(res$padj),
  significance = ifelse(res$padj < 0.01, "significant", "nonsignificant")
)
...
ggplot(data=gdata, aes(x=x, y=y, colour = significance)) +
